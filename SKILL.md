...
description: Use when handling OpenClaw 的“每日提醒”命令式工作流，用户会发送“每日提醒 + 动词”来启动、追加、完成、改时间、查看、停止、查看归档或查看周期汇总，并要求通过飞书进行整点、半点和专项时间提醒。
---
...
- `每日提醒 停止`
- `每日提醒 归档 YYYY-MM-DD`
- `每日提醒 汇总 本周 / 本月 / 最近7天 / 最近30天 / 从 YYYY-MM-DD 到 YYYY-MM-DD`
...
- 清空前会先把当天完整结果归档到本地 archive 目录。
- 归档是只读历史记录，不支持事后修改。

### Show Archive

```bash
python3 {baseDir}/scripts/daily_reminder_state.py show-archive --date 2026-03-26
```
...
```bash
python3 {baseDir}/scripts/daily_reminder_state.py summary --preset this-week
python3 {baseDir}/scripts/daily_reminder_state.py summary --preset this-month
python3 {baseDir}/scripts/daily_reminder_state.py summary --preset last-7-days
python3 {baseDir}/scripts/daily_reminder_state.py summary --preset last-30-days
```

自定义范围：

```bash
python3 {baseDir}/scripts/daily_reminder_state.py summary --from 2026-03-01 --to 2026-03-31
```

自然语言映射规则：

- `本周` -> `--preset this-week`
- `本月` -> `--preset this-month`
- `最近7天` -> `--preset last-7-days`
- `最近30天` -> `--preset last-30-days`
- `从 A 到 B` -> `--from A --to B`

直接把命令返回的 `message` 发给用户。
...
- 不要在模型里手写周报、月报统计逻辑，统一走 archive 查询命令。
- 不要在未经确认时写入模糊任务拆分结果。
